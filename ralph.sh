...
3. Проверь что линтер проходит: npm run lint
4. Проверь что типы проходят: npm run typecheck
5. Проверь что тесты проходят: npm test
6. Обнови статус задачи в tasks.json на "done".
7. Добавь запись в progress.md с датой и ID задачи.
8. Сделай git commit.
